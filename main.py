...
async def ZC(thread):
    ...
    while True:
        ...
        if await zetachain.check_completed_task("NATIVEX_SWAP"):
            status, tx_hash = await retry_function(zetachain.nativex_zeta_to_btc, thread, zetachain.web3_utils.acct.address)
            if status:
                logger.success(f"Поток {thread} | Свапнул на NativeX finance: zeta -> btc! {zetachain.web3_utils.acct.address}:{tx_hash}")
                await zetachain.sleep(config.DELAY['transaction'], logger, thread)
            else:
                logger.error(f"Поток {thread} | Не свапнул на NativeX finance: wzeta -> btc! {zetachain.web3_utils.acct.address}:{tx_hash}")
