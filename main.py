    ...
    while True:
        ...
        if float(await zetachain.allowance_zetaswap_wzeta()) + 0.1 < config.APPROVES['zetaswap_wzeta_approve'][0]:
            ...
            if status:
                ...
                logger.success(
                    f"Поток {thread} | Не апрувнул wzeta для zetaswap! {zetachain.web3_utils.acct.address}:{tx_hash}")
        ...
        if await zetachain.check_completed_task("ZETA_SWAP_SWAP"):
            ...
            if status:
                ...
                logger.success(f"Поток {thread} | Не свапнул на zetaswap: wzeta -> eth.eth! {zetachain.web3_utils.acct.address}:{tx_hash}")
        ...
        if await zetachain.check_completed_task("ZETA_EARN_STAKE"):
            ...
            if status:
                ...
                logger.success(
                    f"Поток {thread} | Не застейкал zeta! {zetachain.web3_utils.acct.address}:{tx_hash}")

        # минт бейджа на ultiverse
        if await zetachain.check_completed_task("ULTIVERSE_MINT_BADGE") and not await zetachain.check_ultiverse_badge():
            ...
            if status:
                ...
                logger.success(
                    f"Поток {thread} | Не сминтил бейдж на ultiverse! {zetachain.web3_utils.acct.address}:{tx_hash}")
