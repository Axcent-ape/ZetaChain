...
class ZetaChain:
    ...
    async def check_completed_task(self, task):
        ...
        while e_count < 2:
            try:
                ...
                logger.error(f"Поток {self.thread} | Ошибка во время проверки выполнения задания {task}!")
                e_count += 1
                await asyncio.sleep(random.uniform(config.DELAY['retry'][0], config.DELAY['retry'][1]))
        else:
            return False
    ...
    async def nativex_zeta_to_btc(self):
        headers = {"apiKey": config.NATIVE_API_KEY}
        random_value = round(random.uniform(config.NATIVEX['zeta_count'][0], config.NATIVEX['zeta_count'][1]), 4)
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.get('https://newapi.native.org/v1/firm-quote', params=params) as resp:
                ...
                to = resp_json.get('txRequest').get('target')
                data = resp_json.get('txRequest').get('calldata')
                value = resp_json.get('txRequest').get('value')
    ...
    async def send_zeta(self):
        if await self.check_completed_task("SEND_ZETA") and config.SENDS_QUESTS['send_zeta'][1]:
            status, tx_hash, random_value = await self.retry_function(self.transfer_zeta, self.thread, self.web3_utils.acct.address)
            if status:
                logger.success(f"Поток {self.thread} | Отправил {random_value} zeta сам себе! {self.web3_utils.acct.address}:{tx_hash}")
                await self.sleep(config.DELAY['transaction'], logger, self.thread)
            else:
                logger.error(f"Поток {self.thread} | Не отправил zeta сам себе! {self.web3_utils.acct.address}:{tx_hash}")

    async def receive_bnb(self):
        if await self.check_completed_task("RECEIVE_BNB") and config.SENDS_QUESTS['send_bnb'][1]:
            status, tx_hash, random_value = await self.retry_function(self.transfer_bnb, self.thread, self.web3_utils.acct.address)
            if status:
                logger.success(
                    f"Поток {self.thread} | Сделал транзакцию: {random_value} zeta -> bnb.bsc! {self.web3_utils.acct.address}:{tx_hash}")
                await self.sleep(config.DELAY['transaction'], logger, self.thread)
            else:
                logger.error(
                    f"Поток {self.thread} | Не сделал транзакцию: zeta -> bnb.bsc! {self.web3_utils.acct.address}:{tx_hash}")

    async def receive_eth(self):
        if await self.check_completed_task("RECEIVE_ETH") and config.SENDS_QUESTS['send_eth'][1]:
            status, tx_hash, random_value = await self.retry_function(self.transfer_eth, self.thread, self.web3_utils.acct.address)
            if status:
                logger.success(f"Поток {self.thread} | Сделал транзакцию: {random_value} zeta -> eth.eth! {self.web3_utils.acct.address}:{tx_hash}")
                await self.sleep(config.DELAY['transaction'], logger, self.thread)
            else:
                logger.error(f"Поток {self.thread} | Не сделал транзакцию: zeta -> eth.eth! {self.web3_utils.acct.address}:{tx_hash}")

    async def receive_btc(self):
        if await self.check_completed_task("RECEIVE_BTC") and config.SENDS_QUESTS['send_btc'][1]:
            status, tx_hash, random_value = await self.retry_function(self.transfer_btc, self.thread, self.web3_utils.acct.address)
            if status:
                logger.success(f"Поток {self.thread} | Сделал транзакцию: {random_value} zeta -> btc.btc! {self.web3_utils.acct.address}:{tx_hash}")
                await self.sleep(config.DELAY['transaction'], logger, self.thread)
            else:
                logger.error(f"Поток {self.thread} | Не сделал транзакцию: zeta -> btc.btc! {self.web3_utils.acct.address}:{tx_hash}")

    async def pool_deposit_any_pool(self):
        if await self.check_completed_task("POOL_DEPOSIT_ANY_POOL") and self.web3_utils.w3.from_wei(self.web3_utils.balance_of_erc20(self.web3_utils.acct.address, '0x48f80608B672DC30DC7e3dbBd0343c5F02C738Eb'), 'ether') >= config.POOLS['send_bnb']:
            status, tx_hash = await self.retry_function(self.add_liquidity, self.thread, self.web3_utils.acct.address)
            if status:
                logger.success(f"Поток {self.thread} | Добавил ликвидность zeta-bnb! {self.web3_utils.acct.address}:{tx_hash}")
                await self.sleep(config.DELAY['transaction'], logger, self.thread)
            else:
                logger.error(f"Поток {self.thread} | Не добавил ликвидность zeta-bnb! {self.web3_utils.acct.address}:{tx_hash}")

    async def eddy_finance_swap(self):
        if await self.check_completed_task("EDDY_FINANCE_SWAP"):
            status, tx_hash = await self.retry_function(self.swap_zeta_to_stzeta, self.thread, self.web3_utils.acct.address)
            if status:
                logger.success(f"Поток {self.thread} | Сделал свап zeta -> stzeta! {self.web3_utils.acct.address}:{tx_hash}")
                await self.sleep(config.DELAY['transaction'], logger, self.thread)
            else:
                logger.error(f"Поток {self.thread} | Не сделал свап zeta -> stzeta! {self.web3_utils.acct.address}:{tx_hash}")

    async def range_protocol_vault_transaction(self):
        if await self.check_completed_task("RANGE_PROTOCOL_VAULT_TRANSACTION") and await self.get_stzeta_balance() >= config.POOLS['stzeta'] and await self.get_wzeta_balance() >= config.EDDY_SWAP['zeta_to_wzeta'] and await self.allowance_stzeta() >= config.POOLS['stzeta'] and await self.allowance_wzeta() >= config.EDDY_SWAP['zeta_to_wzeta']:
            status, tx_hash = await self.retry_function(self.add_liquidity_range, self.thread, self.web3_utils.acct.address)
            if status:
                logger.success(f"Поток {self.thread} | Добавил ликвидность stzeta-wzeta! {self.web3_utils.acct.address}:{tx_hash}")
                await self.sleep(config.DELAY['transaction'], logger, self.thread)
            else:
                logger.error(f"Поток {self.thread} | Не добавил ликвидность stzeta-wzeta! {self.web3_utils.acct.address}:{tx_hash}")

    async def accumulated_finance_deposit(self):
        if await self.check_completed_task("ACCUMULATED_FINANCE_DEPOSIT"):
            # свап zeta на stzeta accumulated
            if await self.get_balance_stzeta_accumulated_finance() < config.ACCUMULATED_FINANCE['zeta_to_stzeta']:
                status, tx_hash = await self.retry_function(self.swap_zeta_to_stzeta_accumulated_finance, self.thread, self.web3_utils.acct.address)
                if status:
                    logger.success(f"Поток {self.thread} | Свапнул на accumulated finance: zeta -> stzeta! {self.web3_utils.acct.address}:{tx_hash}")
                    await self.sleep(config.DELAY['transaction'], logger, self.thread)
                else:
                    logger.error(f"Поток {self.thread} | Не свапнул на accumulated finance: zeta -> stzeta! {self.web3_utils.acct.address}:{tx_hash}")

            # апрув stzeta accumulated finance
            if float(await self.allowance_stzeta_accumulated_finance()) + 0.1 < config.APPROVES['stzeta_accumulated_approve'][0]:
                status, tx_hash = await self.retry_function(self.approve_stzeta_accumulated_finance, self.thread, self.web3_utils.acct.address)
                if status:
                    logger.success(f"Поток {self.thread} | Апрувнул stzeta для accumulated finance! {self.web3_utils.acct.address}:{tx_hash}")
                    await self.sleep(config.DELAY['transaction'], logger, self.thread)
                else:
                    logger.error(f"Поток {self.thread} | Не апрувнул stzeta! {self.web3_utils.acct.address}:{tx_hash}")

            # свап stzeta accumulated finance на wstzeta
            if await self.allowance_stzeta_accumulated_finance() >= config.ACCUMULATED_FINANCE['stzeta_to_wstzeta'] and await self.get_balance_stzeta_accumulated_finance() >= config.ACCUMULATED_FINANCE['stzeta_to_wstzeta']:
                status, tx_hash = await self.retry_function(self.swap_stzeta_to_wstzeta_accumulated_finance, self.thread, self.web3_utils.acct.address)
                if status:
                    logger.success(f"Поток {self.thread} | Свапнул на accumulated finance: stzeta -> wstzeta! {self.web3_utils.acct.address}:{tx_hash}")
                    await self.sleep(config.DELAY['transaction'], logger, self.thread)
                else:
                    logger.error(f"Поток {self.thread} | Не свапнул на accumulated finance: stzeta -> wstzeta! {self.web3_utils.acct.address}:{tx_hash}")

    async def zeta_swap_swap(self):
        if await self.check_completed_task("ZETA_SWAP_SWAP"):
            status, tx_hash = await self.retry_function(self.zetaswap_wzeta_to_eth, self.thread, self.web3_utils.acct.address)
            if status:
                logger.success(f"Поток {self.thread} | Свапнул на zetaswap: wzeta -> eth.eth! {self.web3_utils.acct.address}:{tx_hash}")
                await self.sleep(config.DELAY['transaction'], logger, self.thread)
            else:
                logger.error(f"Поток {self.thread} | Не свапнул на zetaswap: wzeta -> eth.eth! {self.web3_utils.acct.address}:{tx_hash}")

    async def zeta_earn_stake(self):
        if await self.check_completed_task("ZETA_EARN_STAKE"):
            status, tx_hash = await self.retry_function(self.stake_on_zetachain, self.thread), self.web3_utils.acct.address
            if status:
                logger.success(f"Поток {self.thread} | Застейкал zeta! {self.web3_utils.acct.address}:{tx_hash}")
                await self.sleep(config.DELAY['transaction'], logger, self.thread)
            else:
                logger.error(f"Поток {self.thread} | Не застейкал zeta! {self.web3_utils.acct.address}:{tx_hash}")

    async def ultiverse_mint_badge(self):
        if await self.check_completed_task("ULTIVERSE_MINT_BADGE"):
            status, tx_hash = await self.retry_function(self.min_ultiverse_badge, self.thread, self.web3_utils.acct.address)
            if status:
                logger.success(f"Поток {self.thread} | Сминтил бейдж на ultiverse! {self.web3_utils.acct.address}:{tx_hash}")
                await self.sleep(config.DELAY['transaction'], logger, self.thread)
            else:
                logger.error(f"Поток {self.thread} | Не сминтил бейдж на ultiverse! {self.web3_utils.acct.address}:{tx_hash}")

    async def nativex_swap(self):
        if await self.check_completed_task("NATIVEX_SWAP"):
            status, tx_hash = await self.retry_function(self.nativex_zeta_to_btc, self.thread, self.web3_utils.acct.address)
            if status:
                logger.success(f"Поток {self.thread} | Свапнул на NativeX finance: zeta -> btc! {self.web3_utils.acct.address}:{tx_hash}")
                await self.sleep(config.DELAY['transaction'], logger, self.thread)
            else:
                logger.error(f"Поток {self.thread} | Не свапнул на NativeX finance: wzeta -> btc! {self.web3_utils.acct.address}:{tx_hash}")
